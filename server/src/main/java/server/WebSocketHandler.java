...
public class WebSocketHandler {
    ...
    private void makeMove(Session session, MakeMoveCommand command) throws Exception{
        ...
        if(username.equals(gameData.whiteUsername())) color = ChessGame.TeamColor.WHITE;
        else if(username.equals(gameData.blackUsername())) color = ChessGame.TeamColor.BLACK;
        else {sendError(session, "Error: observers cannot make moves"); return;}

        if(game.getTeamTurn() != color){sendError(session, "Error: please wait for your turn"); return;}
        try{
